...
import { CalendarDays, Plus, Clock, Settings, X } from 'lucide-react'
...
export function Sidebar() {
  const { openCreateModal, openSettingsModal, isSidebarOpen, setSidebarOpen } = useUiStore()
  ...
  return (
    <>
      {/* 데스크톱 사이드바 */}
      <aside className="hidden md:flex w-64 border-r border-border flex flex-col bg-background shrink-0">
      {/* 로고 */}
      ...
      {isSidebarOpen && (
        <>
          ...
          />
          {/* 드로어 */}
          <aside className="fixed left-0 top-0 bottom-0 w-64 z-40 bg-background border-r border-border flex flex-col shadow-lg md:hidden">
            {/* 닫기 버튼 */}
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-1.5">
                ...
                {!upcomingLoading && upcomingSchedules && upcomingSchedules.map((schedule) => (
                  <SidebarUpcomingItem key={schedule.id} schedule={schedule} onClose={() => setSidebarOpen(false)} />
                ))}
              </div>
            </div>
...
function SidebarUpcomingItem({ schedule, onClose }: { schedule: Schedule; onClose?: () => void }) {
